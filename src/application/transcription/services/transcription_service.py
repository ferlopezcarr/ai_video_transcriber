...
def transcribe(
    url: str,
    video_name: str | None,
    audio_transcriber_model: str = "faster-whisper",
    lang: str = "en",
):
    """
    ...
    def _getAudioTranscriber(audio_transcriber_model: str):
        if audio_transcriber_model == "openai-whisper":
            from src.infrastructure.outbound.transcriber.adapters.openai_whisper_audio_transcriber import (
                OpenAiWhisperAudioTranscriberAdapter,
            )

            return OpenAiWhisperAudioTranscriberAdapter()
        else:
            from src.infrastructure.outbound.transcriber.adapters.faster_whisper_audio_transcriber import (
    ...
    def _saveTranscription(transcription: str) -> str:
        ...
        return file_storage.save(
            data=transcription, file_path=f"transcriptions/{transcription_file_name}.txt"
        )
    ...
    audio_transcriber: AudioTranscriberPort = _getAudioTranscriber(audio_transcriber_model)
